...
ggplot(sintomas_tempo_melt, aes(x = Data, y = value*100, color = sintoma)) +
  ...
  xlab("M?s") +
  ylab("Frequ?ncia (%)")+
  labs(title = "Frequ?ncia de Sintomas ao Longo do Tempo") +
  theme(plot.title = element_text(margin = margin(t = 20, r = 0, b = 20, l = 0), 
                                  size = 20, color = "black", hjust = 0.5)) +
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 20), size = 15)) +
  theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 20, l = 0), size = 15))
...
####Colocar as regi?es da tabela pela mesma ordem que a dos poligonos
...
incidencia_total <- as.data.frame(data$confirmados_novos/ (popula??o_pt - data$confirmados - data$obitos))
...
##Remover valores negativos devido a erro na base de dados original em que valor cumulativo do n?mero casos homens e mulheres era 0 e n?o devia
...
##INCID?NCIA POR REGI?O LONGO DO TEMPO
...
###Criar uma tabela com uma coluna para as regi?es e outra para o n?mero de pessoas nessa regi?o
...
###Da tabela anterior com todas as datas, selecionar apenas o valor mais recente e mudar nome coluna e regi?es
...
##MORTES POR REGI?O
###Criar tabela com uma coluna para as regi?es e outra para o n?mero mais recente total de ?bitos e dar nomes
...
###Fazer gr?fico com regi?es no eixo do x e n?mero de mortes no eixo do y
...
###Criar tabela com uma coluna para as regi?es e outra para a mortalidade mais recente e dar nomes apropriados
...
###Fazer gr?fico com regi?es no eixo do x e mortaldiade no eixo do y
...
###Juntar as 3 tabelas numa s? mudando os nomes de cada coluna
...
ggplot(letalidade_regioes_tempo_melt, aes(x = Data, y = value*100, color = variable)) +
  ...
  xlab("M?s") +
  ylab("Letalidade (%)") +
  labs(title = "Letalidade por Regi?es ao Longo do Tempo") +
  theme(plot.title = element_text(margin = margin(t = 20, r = 0, b = 20, l = 0), 
                                  size = 20, color = "black", hjust = 0.5)) +
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 20, b = 0, l = 20), size = 15)) +
  theme(axis.title.x = element_text(margin = margin(t = 20, r = 0, b = 20, l = 0), size = 15)) +
